...
using E_commerceManagementSystem.BLL.Manager.PaymentWebhookManager;
...
namespace E_Commerce_System.Controllers
{
    ...
    {
        private readonly IPaymentWebhookManager _paymentWebhookManager;

        public PaymentWebhookController(IPaymentWebhookManager paymentWebhookManager)
        {
            _paymentWebhookManager = paymentWebhookManager;
        }
        ...
        public async Task<IActionResult> PaymobWebhook()
        {
            ...
            var result = await _paymentWebhookManager.HandlePaymobAsync(rawBody, Request.Headers);
            if(result)
              return Ok(result);
            return BadRequest();
        }
